 ...
 */
static const char* BITCOIN_PID_FILENAME = "linkusdd.pid";
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-sporkaddr=<linkusdaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by LINKUSD Platform, to the specified username.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AmericanPrime/LinkUSD.git>";
...
/** Sanity checks
 *  Ensure that LINKUSD Core is running in a usable environment with all
...
{
    // Make sure only a single LINKUSD Core process is using the data directory.
...
bool AppInitMain(const CoreContext& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if LINKUSD Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if LINKUSD Core is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !args.GetBoolArg("-txindex", DEFAULT_TXINDEX) && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/AmericanPrime/LinkUSD.git/pull/1817 and https://github.com/AmericanPrime/LinkUSD.git/pull/1743
    ...
    // ********************************************************* Step 7d: Setup other LINKUSD services
    ...
    // ********************************************************* Step 10a: schedule LINKUSD-specific tasks
