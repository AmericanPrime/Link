...
const char * const BITCOIN_CONF_FILENAME = "linkusd.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //linkusd-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\LINKUSD
    // macOS: ~/Library/Application Support/LINKUSD
    // Unix-like: ~/.linkusd
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LINKUSD";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/LINKUSD";
#else
    // Unix-like
    return pathRet / ".linkusd";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty linkusd.conf if it does not exist
